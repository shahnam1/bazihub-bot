...
CALLBACK_URL  = os.getenv("CALLBACK_URL", "https://zarinpal.com/callback")  # ✅ اصلاح شد
...
CALLBACK_URL = "https://zarinpal.com/callback"  # ✅ اصلاح: یک URL معتبر واقعی
...
def calc_price_tmn_from_usd(product_id: str, usd_price: float, usdt_tmn: float) -> int:
    """
    ✅ اصلاح: محاسبه صحیح قیمت
    1. قیمت دلار × ضریب Liogames (1.0325)
    2. + سود متغیر (از PROFIT_USD_MAP)
    3. × نرخ تومان
    4. گرد به بالاترین هزار تومان
    """
    # مرحله 1: ضریب Liogames
    usd_price = usd_price * LIO_MULTIPLIER

    # مرحله 2: سود دلاری (متغیر براساس محصول)
    profit_usd = PROFIT_USD_MAP.get(product_id, 0.35)
    usd_price = usd_price + profit_usd

    # مرحله 3: تبدیل به تومان
    tmn = usd_price * usdt_tmn

    # مرحله 4: گرد به بالاترین هزار تومان
...
def create_zarinpal_payman_link(amount_tmn: int, description: str) -> Optional[str]:
    ...
    try:
        ...
        # در اینجا تلاش می‌کنیم چند ف  لد متداول را بررسی کنیم.
        data = j.get("data") or {}
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_mlbb":
        ...
        # ✅ اصلاح: پارامترها به ترتیب صحیح
        price_tmn = calc_price_tmn_from_usd(prod.pid, usd, usdt)

        # 🔴 اینجا اضافه کن - مرحله اول (انتخاب محصول)
        logger.info(f"[MENU] محصول: {prod.name}, دلار: {usd:.4f}, نرخ تومان: {usdt:,.0f}, قیمت نهایی: {price_tmn:,}")
        ...
        await q.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️   رگشت", callback_data="menu_mlbb")]])
        )
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get("awaiting_mlbb_id"):
        ...
        # 🔴 اینجا اضافه کن - مرحله دوم (ورود آیدی)
        logger.info(f"[INPUT_ID] محصول: {product_name}, قیمت ذخیره شده: {price_tmn:,}")
...
async def cmd_setprofit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global PROFIT_USD_MAP
    if not context.args:
        await update.message.reply_text(f"📊 سود فعلی:\n{PROFIT_USD_MAP}\nاستفاده: /setprofit 156+16:0.50")
        return
    try:
        # فرمت: /setprofit product_id:profit_usd
        # مثال: /setprofit 156+16:0.50
        parts = context.args[0].split(":")
        if len(parts) != 2:
            await update.message.reply_text("فرمت: /setprofit product_id:profit_usd\nمثال: /setprofit 156+16:0.50")
            return
        pid, profit = parts
        profit = float(profit)
        PROFIT_USD_MAP[pid] = profit
        await update.message.reply_text(f"✅ سود محصول {pid} روی {profit}$ تنظیم شد.")
    except ValueError:
        await update.message.reply_text("❌ لطفاً عدد معتبر وارد کن.")
